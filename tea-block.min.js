...
function encrypt(plaintext, password) {
  ...
  var v = strToLongs(utf8Encode(plaintext)); //  k is 4-word key; simply convert first 16 chars of password as key

  var k = strToLongs(utf8Encode(password).slice(0, 16));
  var cipher = encode(v, k); // convert array of longs to string

  var ciphertext = longsToStr(cipher); // convert binary string to base64 ascii for safe transport

  var cipherbase64 = base64Encode(ciphertext);
  return cipherbase64;
}
/**
...
function decrypt(ciphertext, password) {
  ...
  var v = strToLongs(base64Decode(ciphertext)); //  k is 4-word key; simply convert first 16 chars of password as key

  var k = strToLongs(utf8Encode(password).slice(0, 16));
  var plain = decode(v, k);
  var plaintext = longsToStr(plain); // strip trailing null chars resulting from filling 4-char blocks:
